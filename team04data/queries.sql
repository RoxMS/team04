...
SELECT week, COUNT(DISTINCT orderID) AS total_orders 
FROM orders 
GROUP BY week; 
...
SELECT hour, COUNT(DISTINCT orderID) AS total_orders, SUM(sale) AS total_sales 
FROM orders 
GROUP BY hour; 
...
SELECT item, COUNT(*) AS inventory_items_count   
FROM ingredients
JOIN inventory ON ingredients.item = inventory.ingredient  
GROUP BY item                                  
...
WITH ranked_items AS ( SELECT day, month, year, item, COUNT(item) AS itemsSold, RANK() OVER (PARTITION BY day, month, year 
ORDER BY COUNT(item) DESC) AS rank
    FROM orders
    GROUP BY day, month, year, item
)
SELECT day, month, year, item, itemsSold
FROM ranked_items
WHERE rank = 1;
...
SELECT month, year, SUM(sale) as totalSale          
FROM orders
GROUP BY month, year               
...
SELECT orderID, SUM(sale) AS total_sale 
FROM orders 
GROUP BY orderID 
ORDER BY  total_sale DESC; --sort them in descending order
LIMIT 30;
...
FROM ingredients 
WHERE item = 'Double Stack Cheese Burger'; 
...
SELECT ingredient, SUM(count) as total_count  
FROM ingredients 
GROUP BY ingredient 
...
SELECT SUM(amount) AS total_inventory_amount 
FROM inventory; 
